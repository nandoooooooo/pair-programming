...
describe("a 21 year old employee with a monthly salary of 5900 should have AHV, IV, EO, ALV, NBU and PK deducations", () => {
    ...
    test("6 deducations", () => {
        ...
        const today = new Date()
        const salary: Salary = {
            born: new Date(today.getFullYear()-21, today.getMonth(), today.getDate()),
            payday: new Date(),
            gross: 5900,
        }
        ...
        const alv = salary.gross * (1.1 / 100)
        const nbu = salary.gross * (0.73 / 100)
        const pk = salary.gross * (8.9 / 100)
        const payslip = calculatePayslip(salary)

        // Assert
        expect(payslip.deductions.size).toBe(6)
        expect(payslip.totalDeductions).toBeCloseTo(ahv + iv + eo + alv + nbu + pk)
        expect(payslip.net).toBeCloseTo(salary.gross - (ahv + iv + eo + alv + nbu + pk))
    })
})